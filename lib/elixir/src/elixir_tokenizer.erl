...
tokenize_sigil([H | T], Line, Column, Scope, Tokens, IsDowncase, Acc) when ?is_letter(H) orelse H =:= $. ->
  tokenize_sigil(T, Line, Column, Scope, Tokens, IsDowncase, [H | Acc]);
...
tokenize_sigil([D | T], Line, Column, Scope, Tokens, IsDowncase, Acc) when ?is_delimiter(D) ->
  ...
  case elixir_interpolation:extract(Line, Column + Offset, Scope, IsDowncase, T,
    ...
    {error, Reason} ->
      Sigil = [$~] ++ Name ++ [D],
      interpolation_error(Reason, Sigil ++ T, Scope, Tokens, " (for sigil ~ts starting at line ~B)", [Sigil, Line])
  end;
